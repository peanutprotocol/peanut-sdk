...
export async function isRaffleActive({
  link
}: interfaces.IGetRaffleInfoParams): Promise<boolean> {
...
export async function claimRaffleLink({
  ...
  while (true) {
    ...
    console.log(`Attempting to claim slot ${slotIndexToClaim} out of total ${unclaimedSlots.length} unclaimed slots, slotlink: ${unclaimedSlots[slotIndexToClaim]._slotlink}`)
    ...
    if (response.error || !response.txHash) {
      console.log('Got an error from the relayer while claiming a raffle slot, will retry claiming a different slot', response.error)
      continue
    }
