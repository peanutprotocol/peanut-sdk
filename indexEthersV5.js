...
export function generateKeysFromString(string) {
    ...
    // var privateKey = ethers.utils.keccak256(ethers.toUtf8Bytes(string)); // v6
...
export function solidityHashAddress(address) {
    ...
    return ethers.utils.solidityKeccak256(["address"], [address]); // no longer exists in ethers v6
...
export async function createLink({
    ...
    if (tokenType == 0) {
        ...
        tokenAmount = ethers.utils.parseEther(tokenAmount.toString()); // v5
...
async function makeDeposits(signer, chainId, contractVersion, numberOfLinks, tokenType, tokenAmount, tokenAddress, tokenDecimals, keys) {
    ...
    const maxFeePerGas = ethers.utils.parseUnits('900', 'gwei'); // 100 Gwei // v5
    // const maxPriorityFeePerGas = ethers.parseUnits('50', 'gwei'); // 2 Gwei // v6
    const maxPriorityFeePerGas = ethers.utils.parseUnits('50', 'gwei'); // 2 Gwei // v5
